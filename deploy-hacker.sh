...
echo "📁 Backup tampilan lama..."
mkdir -p backups/$(date +%s)
cp -r src/views/* backups/$(date +%s)/ 2>/dev/null || true

echo "🎨 Menulis ulang layout.ts tema hacker..."
...
echo "🎨 Menulis ulang dashboard.ts (API keys)..."
...
echo ""
echo "✅ Semua tampilan hacker sudah terpasang."
echo "🚀 Melakukan git push untuk deploy otomatis..."
git add -A
git commit -m "Tampilan hacker: dashboard, landing, chat + matrix rain" || echo "Tidak ada perubahan baru."
git push || echo "Gagal push, pastikan remote Git sudah diatur."
echo ""
echo "Selesai. Jika Cloudflare Workers tersambung ke repo ini, worker akan ter-deploy otomatis."
echo "Jangan lupa jalankan schema.sql BARU di D1 Console bila ada tabel baru."
